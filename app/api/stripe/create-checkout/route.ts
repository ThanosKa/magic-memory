...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!parseResult.success) {
      logger.warn(
        { userId, error: "Invalid request body" },
        "Validation failed"
      );
    ...
    if (!pkg?.priceId) {
      logger.error({ packageType }, "Missing Stripe price id for package");
      return NextResponse.json(
    ...
    logger.info(
      { userId, packageType, credits: pkg.credits, price: pkg.price },
    ...
    logger.info(
      { userId, sessionId: session.id, packageType },
